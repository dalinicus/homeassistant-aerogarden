...
class Aerogarden:
    ...
    def __init__(self, host:str, username:str, password:str) -> None:
        ...
        self._client = AerogardenClient(host)
    ...
    def get_garden_name(self, config_id):
        planted_name_decoded = base64.b64decode(self.garden_property(config_id, GARDEN_KEY_PLANTED_NAME)).decode("utf-8")

        is_multi_garden = self.__is_multi_guarden(config_id)
        if not is_multi_garden:
            return planted_name_decoded 
        
        multi_garden_label = "Left" if self.garden_property(config_id, GARDEN_KEY_CHOOSE_GARDEN) == 0 else "Right"
    ...
    def toggle_light(self, config_id):
        ...
        air_guid = self.garden_property(config_id, GARDEN_KEY_AIR_GUID)
        choose_garden = self.garden_property(config_id, GARDEN_KEY_CHOOSE_GARDEN)
        
        # I couldn't find any way to set a specific state, it just cycles between the three.
        plant_config = f'{ "lightTemp" : {self.garden_property(config_id, GARDEN_KEY_LIGHT_TEMP)} }'
        
        self._client.update_device_config(air_guid, choose_garden, plant_config)
        self.update(no_throttle=True)
    ...
    async def update(self):
        try:
            data = {}
            devices = self._client.get_user_devices()
    ...
    def __is_multi_guarden(self, config_id:int) -> bool:
        chooseGuarden = self.garden_property(config_id, GARDEN_KEY_CHOOSE_GARDEN)
        if(chooseGuarden > 0):
        ...
        return any(
            garden[GARDEN_KEY_AIR_GUID] == self.garden_property(config_id, GARDEN_KEY_AIR_GUID) 
                and garden[GARDEN_KEY_CHOOSE_GARDEN] > 0 
                for garden in self._data[config_id]
        )
